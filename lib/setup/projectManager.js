...
export class ProjectManager {
    ...
    setupProjectPackage() {
        ...
        packageJson.main = `src/${this.useTypescript ? 'main.ts' : 'main.js'}`;
    ...
    createBaseFiles() {
        const files = [
            { path: '.env', content: fs.readFileSync(path.join(this.__templateDir, '.env'), 'utf-8') },
            { path: '.gitignore', content: fs.readFileSync(path.join(this.__templateDir, '.gitignore'), 'utf-8') }
        ];
